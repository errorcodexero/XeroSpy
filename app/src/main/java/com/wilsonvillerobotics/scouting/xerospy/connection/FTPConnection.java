package com.wilsonvillerobotics.scouting.xerospy.connection;
...
public class FTPConnection {
    ...
    public FTPConnection(Activity act) {
        ...
        xmlFolder.mkdir();
        serverAddress = sharedPreferences.getString("server_ip", "127.0.0.1");
    }
    ...
    */
    ...
    public boolean getFTPFiles(String baseFolder, ManageDBActivity.DataTransferTask task) {
        ...
        if(isConnected()){
            try {
                ...
                if (ftpFiles != null && ftpFiles.length > 0) {
                    ...
                    for (FTPFile file : ftpFiles) {
                        ...
                        if(ftp.retrieveFile(file.getName(), output)) {
                            fileCount++;
                        }
                        //close output stream
                        output.close();
            ...
            task.myPublish("Successfully pulled " + fileCount + " DB XML Files");
            return true;
        }
        task.myPublish("No network connection");
        return false;
    }
